...
int Screen::S_TEXT_X = 400;
int Screen::S_TEXT_Y = 565;
int Screen::S_TEXT_RECT_WIDTH = 320;
int Screen::S_TEXT_RECT_HEIGHT = 30;
const std::string Screen::S_SCORE_TEXT = "Score: ";
const std::string Screen::S_LIVES_TEXT = "Lives: ";

Screen::Screen()
    : m_window(NULL), m_renderer(NULL), m_texture(NULL), m_mainBuffer(NULL),
      m_sansFont(NULL), m_textSurface(NULL), m_textTexture(NULL)
{
}
...
{
    int paintCol = S_WIDTH / 2;
    int paintRow = S_HEIGHT / 2;
    ...
    for (int i = 0; i < S_HEIGHT / 2 - 1; i++)
        for (int j = 0; j < S_WIDTH / 2 - 1; j++)
        {
            ...
            m_mainBuffer[(i + S_HEIGHT / 2 + 1) * S_WIDTH + j + S_WIDTH / 2 +
                         2] = 0xAD0C0A00;
        }
}
