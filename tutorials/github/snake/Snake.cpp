...
int Snake::S_INITIAL_DIRECTION = Snake::Direction::RIGHT;
int Snake::S_N_SECTS = 8;
int Snake::S_INITIAL_LIVES = 3;

Snake::Snake()
    : m_speed(Snake::S_INITIAL_SPEED), m_lives(Snake::S_INITIAL_LIVES),
      m_direction(S_INITIAL_DIRECTION), m_hasUpdated(false)
{
    ...
    {
        newSection =
            new Section(Screen::S_WIDTH / 2 - i * Section::S_SECTION_WIDTH,
...
{
    if ((m_direction == Snake::Direction::UP ||
...
bool Snake::move()
{
    for (int i = m_sections.size() - 1; i > 0; i--)
        m_sections[i]->move(
            m_sections[i]->calculateDirection(*m_sections[i - 1]));
...
void Snake::addSection()
{
    ...
    int basePlacement =
        m_sections[N - 2]->calculateDirection(*m_sections[N - 1]);
