...
OptionsState::OptionsState()
{
    ...
    title = new Text("font/8-BIT WONDER.ttf",
                     50,
...
void OptionsState::update(float)
{
    ...
    if (pressed[START] || pressed[A])
    {
        ...
        else
        {
            if (current_option == 0)
            { // screen resolution
                vector<pair<int, int>> resolutions = {ii(800, 600),
                                                      ii(1024, 768),
                                                      ii(1280, 720),
                                                      ii(1366, 768),
                                                      ii(1920, 1080)};
    ...
    for (int i = 0; i < (int)options.size(); i++)
    {
        ...
        if (i)
            prev_text_size =
                max((int)sub_options[options[i - 1]->get_text()].size(), 1);
...
void OptionsState::build_options()
{
    ...
    sub_options["FULLSCREEN"].push_back(new Text("font/8-BIT WONDER.ttf",
                                                 30,
                                                 Text::TextStyle::SOLID,
                                                 "OFF",
                                                 WHITE));
