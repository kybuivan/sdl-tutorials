                         ...
                         vector<pair<string, string>> players_info)
{
    ...
    for (int i = 0; i < (int)players_info.size(); i++)
    {
        ...
        if (char_name == "blood")
        {
            players[i] = new Blood(skin_name,
                                   char_positions[i].first,
                                   char_positions[i].second,
                                   i);
        }
        else if (char_name == "flesh")
        {
            players[i] = new Flesh(skin_name,
                                   char_positions[i].first,
                                   char_positions[i].second,
                                   i);
        }
    ...
    {
        add_object(new FighterStats(players[i],
                                    i + 1,
                                    i > 1,
...
void BattleState::update(float delta)
{
    ...
    if (time_counter->is_over() && !game_over)
    {
        game_over = true;
    ...
    if (!alive[0] && !alive[1] && !alive[2] && !alive[3] && !game_over)
    {
        game_over = true;
        battleEnd = new BattleEnd(3);
        add_object(battleEnd);
    ...
    if (!alive[0] && !alive[1] && !game_over)
    {
        game_over = true;
        battleEnd = new BattleEnd(2);
        add_object(battleEnd);
    }
    else if (!alive[2] && !alive[3] && !game_over)
    {
        game_over = true;
        battleEnd = new BattleEnd(1);
        add_object(battleEnd);
