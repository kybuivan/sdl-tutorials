...
void CharacterSelectState::update(float delta)
{
    ...
    if (pressed[FIRST_PLAYER][SELECT] ||
        (!selected[FIRST_PLAYER] && pressed[FIRST_PLAYER][B]))
    {
        selected_sound.play();
        m_quit_requested = true;
    ...
    for (int i = 0; i < N_PLAYERS; i++)
    {
        if (!selected[i])
        {
            ...
            if (pressed[i][Y])
            {
                ...
                } while (!chars[char_sel].is_enabled());

                do
                {
                    rand_skin = rand() % N_SKINS;
                } while (!chars[char_sel].is_skin_available(rand_skin));
            ...
            if (pressed[i][A])
            {
                ...
                {
                    if (!chars[char_sel].is_skin_available(cur_skin[i]))
                    {
                        blocked.play();
                    }
                    else
...
void CharacterSelectState::render()
{
    ...
    for (int i = 0; i < N_PLAYERS; i++)
    {
        ...
        if (!char_selected.is_skin_available(cur_skin[i]) && !selected[i])
        {
