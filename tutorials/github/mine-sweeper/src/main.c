...
SDL_Color numColors[NUMBER_TEXTURES] = {{0x20, 0x20, 0xFF, 0xFF},
                                        {0x20, 0xFF, 0x20, 0xFF},
                                        {0xFF, 0x20, 0x20, 0xFF},
                                        {0x00, 0x00, 0x8B, 0xFF},
                                        {0xA5, 0x2A, 0x2A, 0xFF},
...
static TextTexture *loadTextTexture(TTF_Font *font,
                                    const char *str,
                                    SDL_Color color);
...
static bool init(int argc, char **argv)
{
    ...
    int width = DEFAULT_MAP_WIDTH, height = DEFAULT_MAP_HEIGHT,
        mines = DEFAULT_MAP_MINES;
    if (argc > 3)
    {
    ...
    else
    {
        ...
        gWindow = SDL_CreateWindow("Mine Sweeper",
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        printf(
                            "Unable to load icon image! SDL_image Error: %s\n",
                            IMG_GetError());
                    }
                    else
                    {
...
static bool loadMedia()
{
    ...
    if (success)
    {
        ...
        if (mineTexture == NULL || flagTexture == NULL ||
            questionTexture == NULL || explosionTexture == NULL ||
            errorTexture == NULL)
            success = false;
    }
    if (success)
        ...
        else
        {
            ...
            while (!quit)
            {
                ...
                while (SDL_PollEvent(&e) != 0)
                {
                    ...
                    else if (e.type == SDL_MOUSEBUTTONDOWN)
                    {
                        if (!game_over)
                        {
                            ...
                            {
                                markMap(map,
                                        e.button.y / BLOCK_SIZE,
                                        e.button.x / BLOCK_SIZE);
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < map->height; ++i)
                {
                    ...
                    {
                        SDL_Rect iconRect = {(int)((j + 0.125) * BLOCK_SIZE),
                        ...
                        {
                            bool wrongMarkMine =
                                !isMine && status == MAP_STATUS_MARK_MINE;
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    TextTexture *num =
                                        numTextures[indicator - 1];
                                    SDL_Rect rect = {
                                        j * BLOCK_SIZE +
                                            (BLOCK_SIZE - num->width) / 2,
                                        i * BLOCK_SIZE +
                                            (BLOCK_SIZE - num->height) / 2,
                                        num->width,
                                        num->height};
                                    SDL_RenderCopy(gRenderer,
                                                   num->texture,
