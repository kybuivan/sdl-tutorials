...
{
    if (SDL_SetRenderDrawColor(mRenderer.get(),
                               color.red(),
                               color.green(),
                               color.blue(),
                               color.alpha()) < 0)
...
auto Renderer::copy(Texture const &texture,
                    ...
                    std::optional<Rectangle> const &destinationBounds) noexcept
    -> void
{
    auto const srcrect = [&]() {
    ...
    if (SDL_RenderCopy(mRenderer.get(),
...
Renderer::Renderer() noexcept
    : mRenderer{SDL_CreateRenderer(Window::instance().getRawPointer(),
                                   -1,
