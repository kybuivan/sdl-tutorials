...
SDL_Rect GameTitleFillRect{GameTitle_xOffset,
                           GameTitle_yOffset,
                           GameTitle_Width,
                           GameTitle_Height};
...
SDL_Rect GameInfoFillRect{GameInfo_xOffset,
                          GameInfo_yOffset,
                          GameInfo_Width,
                          GameInfo_Height};
...
SDL_Rect levelCompleteFillRect{LevelComplete_xOffset,
                               LevelComplete_yOffset,
                               LevelComplete_Width,
                               LevelComplete_Height};

SDL_Rect levelCompleteTitleRect{levelCompleteFillRect.x,
...
bool initialization()
{
    ...
    {
        //Create window
        window = SDL_CreateWindow("A Simple Maze Game",
...
void DrawTextFromRect(std::string text, SDL_Rect *textRect, int fontSize)
{
    ...
    SDL_Rect DisplayTextRect = {text_xOffset,
                                text_yOffset,
                                textSurface->w,
                                textSurface->h};
...
void DrawTextFromRectParams(std::string text,
                            int xOffset,
                            int yOffset,
                            int width,
                            int height,
                            int fontSize)
{
    ...
    SDL_Rect textRect = {xOffset, yOffset, width, height};
...
void DrawTextWithAdjCenteredNumber(std::string text,
                                   std::string count,
                                   int xOffset,
                                   int yOffset,
                                   int width,
...
void DrawTextMultipleRows(std::vector<std::string> &text,
                          ...
                          int fontSize)
{
    ...
    for (int i = 0; i < rowCount; i++)
    {
        ...
        {
            DrawTextWithAdjCenteredNumber(text[i],
                                          data[i],
                                          xOffset,
                                          yOffset + rowHeight * i,
                                          width,
...
{
    std::vector<std::string> infoTexts = {"Level",
                                          "Steps",
                                          "Lives",
                                          "R to Reset",
                                          "ESC to Menu"};
    ...
    DrawTextMultipleRows(infoTexts,
                         dataTexts,
                         GameInfo_xOffset,
                         GameInfo_yOffset,
                         GameInfo_Width,
...
{
    DrawInGameGUI(std::to_string(level),
                  std::to_string(steps),
                  std::to_string(lives));
}
...
bool FrontScreen()
{
    ...
    return startEvent->key.keysym.sym == SDLK_ESCAPE ||
           startEvent->type == SDL_QUIT;
}
...
bool LevelCompleteScreen(int steps, int deaths)
{
    ...
    DrawTextFromRect("Level Complete!",
                     &levelCompleteTitleRect,
                     LevelComplete_FontSize);

    //Information in Level Complete Panel
    std::vector<std::string> levelComplete_TextRows = {
        "Steps Taken",
        "Lives Lost",
        "Press Space To Continue"};
    std::vector<std::string> levelComplete_DataRows = {std::to_string(steps),
                                                       std::to_string(deaths)};

    //Add Information To Renderer in Multiple Rows
    DrawTextMultipleRows(levelComplete_TextRows,
                         levelComplete_DataRows,
                         levelCompleteInfoRect.x,
                         levelCompleteInfoRect.y,
                         levelCompleteInfoRect.w,
                         levelCompleteInfoRect.h,
                         42);
    SDL_RenderPresent(renderer);
    SDL_Event *continueEvent = new SDL_Event;

    //Check for User Input to see what to do next
    while (continueEvent->key.keysym.sym != SDLK_SPACE &&
           continueEvent->type != SDL_QUIT)
    {
        //Do something when there is an event
...
void RenderAllGameObjects(std::shared_ptr<Maze> &maze,
...
void ResetToStart(std::shared_ptr<Maze> &maze,
                  std::shared_ptr<Player> &player,
                  int *level,
                  int *steps,
                  int *lives)
        ...
        while (!quit && gameEvent->type != SDL_QUIT && curGameState >= Menu)
        {
            ...
            while (!quit && gameEvent->type != SDL_QUIT && curGameState == Menu)
            {
                ...
                else
                {
                    ...
                    {
                        curMaze =
                            std::shared_ptr<Maze>(new Maze(mazeX,
                                                           mazeY,
                                                           GameArea_xOffset,
                                                           GameArea_yOffset,
                                                           GameArea_Width,
                                                           ...
                                                           level,
                                                           false));
                        curPlayer = std::shared_ptr<Player>(new Player(
                            curMaze->FindRoomByPos(curMaze->startPos),
                            renderer));
                   ...
                   curGameState == InGame)
            {
                ...
                if (gameEvent->type == SDL_KEYDOWN)
                {
                    ...
                    else if (keyInput == SDLK_r && curPlayer->playerLives > 1)
                    {
                        ...
                        curMaze->ResetMaze();
                        curPlayer->SetPlayerToStart(
                            curMaze->FindRoomByPos(curMaze->startPos));
                    }

                    //If the player presses an arrow key, check for a movement
                    else if (keyInput == SDLK_DOWN || keyInput == SDLK_LEFT ||
                             keyInput == SDLK_RIGHT || keyInput == SDLK_UP)
                    {
                   ...
                   curGameState == LevelComplete)
            {
                ...
                quit = LevelCompleteScreen(
                    steps,
                    std::max(0, levelStartLives - curPlayer->playerLives));
                if (quit)
                {
                    break;
                }
