...
public:
    ...
    void CreateRooms()
    {
        ...
        for (int x = 0; x < mazeX_RoomCount; x++)
        {
            ...
            {
                std::shared_ptr<Room> newPtr(
                    new Room(Coordinate(x, y), mazeRenderer));
                newPtr->MakeRoomRect(mazeX_Offset,
                                     mazeY_Offset,
                                     mazeX_RoomCount,
                                     mazeY_RoomCount,
                                     mazeX_Size,
                 ...
                 [&](std::shared_ptr<Room> &curRoomPtr) {
                     std::for_each(
                         ...
                         [&](directions dir) {
                             ...
                             auto adjRoom = find_if(
                                 begin(allRooms),
                                 end(allRooms),
                                 [&](std::shared_ptr<Room> &checkRoomPtr) {
    ...
    void CarveMaze()
    {
        ...
        while (inMazeCounter < mazeX_RoomCount * mazeY_RoomCount)
        {
            ...
            if (!curRoomPtr->inMaze)
            {
                ...
                for_each(begin(curRoomPtr->adjRooms),
                         end(curRoomPtr->adjRooms),
                         [&](std::shared_ptr<Room> adjRoomPtr) {
                             //Find if the current room is in the available rooms of the current adjacent room
                             auto roomToRemoveIter =
                                 std::find(begin(adjRoomPtr->availRooms),
                                           end(adjRoomPtr->availRooms),
                                           curRoomPtr);
            ...
            if (!curRoomPtr->availRooms.empty())
            {
                ...
                std::shared_ptr<Room> nextRoomPtr =
                    randomElement(curRoomPtr->availRooms);
                ...
                curRoomPtr->availRooms.erase(
                    std::find(begin(curRoomPtr->availRooms),
                              end(curRoomPtr->availRooms),
                              nextRoomPtr));
        ...
        for_each(
            ...
            [&](std::shared_ptr<Room> curRoomPtr) {
                if (curRoomPtr->roomTypes.size() == 0)
                {
                    ...
                    if (curRoomPtr->connectRooms.size() == 3 &&
                        mazeLevel > GUARDS_START)
                    {
                        std::shared_ptr<MazeGuard> mazeGuardPtr =
                            std::shared_ptr<MazeGuard>(
                                new MazeGuard(curRoomPtr));
                        objectsInMaze.push_back(
                            std::static_pointer_cast<MazeObject>(mazeGuardPtr));
                    }
                    //Otherwise, if the difficulty is 1 or higher, create a Trap
                    else
                    {
                        std::shared_ptr<MazeTrap> mazeTrapPtr =
                            std::shared_ptr<MazeTrap>(new MazeTrap(curRoomPtr));
                        objectsInMaze.push_back(
                            std::static_pointer_cast<MazeObject>(mazeTrapPtr));
                    }
                }
            });
    }
    ...
    {
        auto iter = (std::find_if(begin(allRooms),
                                  end(allRooms),
                                  [&](std::shared_ptr<Room> checkRoom) {
    ...
    void AddMazeObstaclesToRenderer()
    {
        if (mazeLevel > TRAPS_START)
        {
            for_each(begin(objectsInMaze),
                     end(objectsInMaze),
                     [&](std::shared_ptr<MazeObject> curObjPtr) {
