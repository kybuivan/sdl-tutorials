...
public:
    ...
    bool PlayerMove(SDL_Keycode key, SDL_Renderer *renderer)
    {
        ...
        auto iter = std::find_if(begin(curObjRoom->connectRooms),
                                 end(curObjRoom->connectRooms),
                                 [&testPos](std::shared_ptr<Room> nextRoom) {
    ...
    {
        //Pick Up Key
        if (std::find(begin(curObjRoom->roomTypes),
                      end(curObjRoom->roomTypes),
                      Key) != end(curObjRoom->roomTypes))
        {
            hasKey = true;
        ...
        else if (std::find(begin(curObjRoom->roomTypes),
                           end(curObjRoom->roomTypes),
                           Guard) != end(curObjRoom->roomTypes))
        {
            SetObjRoom(startRoom);
